...
class _TelegramSpamPageState extends State<TelegramSpamPage>
  ...
  // --- Color Scheme ---
  static const Color _primaryGreen = Color(0xFF4DA3FF);
  static const Color _darkBg = Color(0xFF070317);
  static const Color _cardBg = Color(0xFF120A2B);
  static const Color _inputBg = Color(0xFF101E3C);
  ...
  Future<void> _loadSessions() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/sessions?key=${widget.sessionKey}',
        ),
      );
  ...
  Future<void> _initiateLogin({bool isResend = false}) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/login?key=${widget.sessionKey}&phone=$phone',
        ),
      );
  ...
  Future<void> _submitAuth() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/auth?key=${widget.sessionKey}&loginId=$_currentLoginId&input=${_authController.text.trim()}',
        ),
      );
  ...
  void _startLoginStatusPolling() {
    ...
    _loginStatusTimer = Timer.periodic(const Duration(seconds: 2), (
      ...
      try {
        final response = await http.get(
          Uri.parse(
            'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/status?key=${widget.sessionKey}&loginId=$_currentLoginId',
          ),
        );
        final data = jsonDecode(response.body);
  ...
  Future<void> _verifySessionPassword(String phone) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/verify-session-password',
        ),
  ...
  Future<void> _refreshSessions() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/refresh-sessions?key=${widget.sessionKey}',
        ),
      );
      final data = jsonDecode(response.body);
      if (data['valid'] == true) {
  ...
  Future<void> _deleteSession(String phone) async {
    try {
      final response = await http.get(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/remove-ses?key=${widget.sessionKey}&phone=$phone',
        ),
      );
      final data = jsonDecode(response.body);
      if (data['valid'] == true) {
  ...
  Future<void> _startSpamReport() async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
          'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/spam-report',
        ),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'key': widget.sessionKey,
  ...
  void _startStatusPolling() {
    ...
    _statusCheckTimer = Timer.periodic(const Duration(seconds: 2), (
      ...
      try {
        final response = await http.get(
          Uri.parse(
            'http://tirz.panel.jserver.web.id:2001/api/tools/telegram/report-status?key=${widget.sessionKey}&reportId=$_currentReportId',
          ),
        );
        final data = jsonDecode(response.body);
        if (data['valid'] == true) {
  ...
  void _showSnackBar(String message, {bool isError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? const Color(0xFF2C1B5E) : _primaryGreen,
        ...
        title: Row(
          children: [
            Icon(
              ...
              color: isBanned ? _primaryGreen : const Color(0xFF6B8CFF),
              size: 24,
            ),
                  ...
                  children: [
                    ...
                    if (_loginErrorMessage.isNotEmpty)
                      Padding(
                        ...
                        child: Text(
                          _loginErrorMessage,
                          style: const TextStyle(color: const Color(0xFF5B4CFF)),
                          textAlign: TextAlign.center,
                        ),
                      ),
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(
          color: widget.session.isActive
              ...
              : const Color(0xFF7A5CFF).withOpacity(0.3),
        ),
      ),
      child: Column(
        children: [
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      obscureText: true,
                      style: const TextStyle(color: Color(0xFF4DA3FF)),
                      decoration: InputDecoration(
                        ...
                        labelStyle: const TextStyle(color: Color(0xFF4DA3FF)),
                        prefixIcon: const Icon(
                          Icons.lock,
                          color: Color(0xFF4DA3FF),
                        ),
                        filled: true,
                        fillColor: const Color(0xFF101E3C),
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(10),
                          borderSide: BorderSide.none,
                        ),
...
class ReportInputCard extends StatelessWidget {
  ...
  InputDecoration _inputDecoration(String label, String hint, IconData icon) {
    return InputDecoration(
      ...
      hintStyle: TextStyle(color: Colors.white.withOpacity(0.5)),
      labelStyle: const TextStyle(color: Color(0xFF4DA3FF)),
      prefixIcon: Icon(icon, color: const Color(0xFF4DA3FF)),
      filled: true,
      fillColor: const Color(0xFF101E3C),
      border: OutlineInputBorder(
        borderRadius: BorderRadius.circular(10),
        ...
        child: Column(
          ...
          children: [
            ...
            LinearProgressIndicator(
              ...
              backgroundColor: const Color(0xFF3A3D6B),
