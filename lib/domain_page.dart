...
class _DomainOsintPageState extends State<DomainOsintPage> {
  ...
  final Color accentPurple = const Color(0xFF5E86FF);
  // Warna kartu sedikit bernuansa ungu sangat gelap
  final Color cardDark = const Color(0xFF120A2B);
  ...
  List<Widget> _buildSubdomainsList() {
    ...
    final cleanSubdomains =
        _subdomainsData!
            .map((item) => item.toString().split('\n').last.trim())
            .where(
              (subdomain) => subdomain.isNotEmpty && !subdomain.startsWith('*'),
            )
                      ...
                      children: [
                        ...
                        if (_dnsData != null && _dnsData!['records'] != null)
                          _buildCategoryCard(
                            ...
                            icon: Icons.dns,
                            children: [
                              if (_dnsData!['records']['a']?['query']?['server'] !=
                                  null)
                                _buildInfoRow(
                                  label: "DNS Server",
                                  value:
                                      _dnsData!['records']['a']!['query']!['server']!['ip']
                                          ?.toString(),
                                  showCopyButton: true,
                                ),
                              if (_dnsData!['records']['a']?['query']?['server']?['location'] !=
                                  null)
                                _buildInfoRow(
                                  label: "Lokasi Server",
                                  value:
                                      "Lat: ${_dnsData!['records']['a']!['query']!['server']!['location']!['lat']}, Lon: ${_dnsData!['records']['a']!['query']!['server']!['location']!['lon']}",
                                ),
                            ],
                          ),
                      ],
